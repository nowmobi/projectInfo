// 使用 ES6 模块导入
import { getImgUrl, Category_URL } from "./BaseURL.js";
...
class CategoryPage {
  ...
  renderSidebarCategories() {
    ...
    if (!this.categories || this.categories.length === 0) {
      sidebarCategories.innerHTML = '<div class="sidebar-category">Loading categories...</div>';
      return;
    }
  ...
  bindSidebarCategoryEvents() {
    const sidebarCategories = document.querySelectorAll(".sidebar-item[data-category]");
  ...
  getArticleCountByCategory(categoryId) {
    ...
    return this.articles.filter(
      (article) => article.type === categoryId
    ).length;
  }
