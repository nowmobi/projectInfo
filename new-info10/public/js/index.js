import { Category_URL, getCategoryOrder, fetchApiData } from "./BaseURL.js";
...
class Website {
  ...
  async loadData() {
    try {
      ...
      if (Array.isArray(data) && data.length > 0 && data[0]) {
        ...
        } else if (data[0].info2) {
          this.categoryOrder = data[0].info2;
        } else {
          ...
          <div class="article-card" data-id="placeholder-${index}-1">
            ...
            <div class="article-content">
              <div>
                ...
                <div class="article-time">2025-01-01</div>
              </div>
            </div>
          </div>
  ...
  renderHotView() {
    ...
    const adsElements = Array.from(articlesGrid.querySelectorAll(".ads"));
    const adsHTML = adsElements.map((ads) => ads.outerHTML).join("");
    ...
    if (hotSections.length > 0) {
      hotCategories.forEach((categoryData, index) => {
        if (index < hotSections.length) {
          ...
          let titleEl = section.querySelector(".hot-category-title");
          let articlesContainer = section.querySelector(
            ".hot-category-articles"
          );
          ...
          newSection.className = "hot-category-section";
          newSection.innerHTML = `
            <h3 class="hot-category-title">${categoryData.categoryName}</h3>
            <div class="hot-category-articles">
              ${categoryData.articles
                .map((article) => this.createBusinessCard(article, isReverse))
                .join("")}
            </div>
          `;
          articlesGrid.appendChild(newSection);
        }
      });
      ...
      articlesGrid.innerHTML =
        ...
        hotCategories
          .map((categoryData, index) => {
            const isReverse = index % 2 === 1;
            return `
          <div class="hot-category-section">
            <h3 class="hot-category-title">${categoryData.categoryName}</h3>
            <div class="hot-category-articles">
              ${categoryData.articles
                .map((article) => this.createBusinessCard(article, isReverse))
