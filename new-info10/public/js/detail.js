import { Category_URL, getDataBaseUrl, fetchApiData, API_PATH_PART } from "./BaseURL.js";
...
class ArticleDetailPage {
  ...
  async loadArticleDetail() {
    try {
      const articleUrl = `${REMOTE_DATA_BASE_URL}/${this.articleId}/${API_PATH_PART}/data.json`;
  ...
  renderArticle(article) {
    ...
    if (article.author) {
      articleType.textContent = this.decodeUnicode(article.author);
    } else {
    ...
    const articleSourceValue = document.getElementById("articleSourceValue");
    if (article.source) {
      articleSourceValue.textContent = article.source;
    ...
    if (imgElements.length > 0) {
      ...
      if (
        ...
        if (article && article.id) {
          ...
          const remoteImgUrl = `${REMOTE_DATA_BASE_URL}/${article.id}/${imgFileName}`;
  ...
  normalizeContentImages(container, articleId) {
    ...
    images.forEach((img) => {
      ...
      if (normalisedId) {
        img.setAttribute(
          "src",
          `${REMOTE_DATA_BASE_URL}/${normalisedId}/${originalSrc}`
        );
      } else {
  ...
  decodeUnicode(str) {
    ...
    protectedStr = protectedStr.replace(urlRegex, (match, attr, quoteStart, url, quoteEnd) => {
      
      const placeholder = `__URL_PLACEHOLDER_${urlPlaceholders.length}__`;
      
      urlPlaceholders.push({ attr, url, quoteStart, quoteEnd });
      
      return `${attr}=${quoteStart}${placeholder}${quoteEnd}`;
    });
    ...
    urlPlaceholders.forEach(({ attr, url, quoteStart, quoteEnd }, index) => {
      const placeholder = `__URL_PLACEHOLDER_${index}__`;
      decoded = decoded.replace(new RegExp(placeholder, "g"), url);
    });
