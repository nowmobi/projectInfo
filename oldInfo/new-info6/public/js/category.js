...
// Import HealthNewsApp for sidebar categories (same logic as home page)
import { HealthNewsApp } from "./index.js";

// Expose imported variables to global scope for non-module scripts
...
class CategoryPage {
  ...
  async loadData() {
    ...
    while (retryCount < maxRetries) {
      try {
        // 使用 Category_URL 从远程获取数据
        const response = await fetch(window.Category_URL);
  ...
  getArticleImagePath(article) {
    ...
    if (article && typeof article === "object") {
      if (article.img && window.IMG_BASE_URL) {
        
        return window.IMG_BASE_URL.replace(
          "/number/",
          `/${article.id}/`
        ).replace("number.png", article.img);
      }
      ...
      if (imgPath && imgPath.startsWith("public/")) {
        return "../" + imgPath;
      }
      return (
    ...
    if (article.startsWith("http://") || article.startsWith("https://")) {
      return article;
    }
    if (article.startsWith("public/")) {
      return "../" + article;
    }
    return article;
  }
  ...
  async renderCategories() {
    ...
    try {
      ...
      const orderedCategories = defaultOrder
  ...
  // 这些函数已不再需要，因为不再需要切换视图
  // 直接使用 renderCategories() 和 renderArticles() 即可
...
// Custom bindDropdownNavigation for category page
...
// Override initCommonSidebar to use custom navigation
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 初始化 HealthNewsApp 用于侧边栏分类（确保菜单内容和首页一致）
  window.healthNewsApp = new HealthNewsApp({ useHomePageLayout: false });
  
  // 等待 HealthNewsApp 初始化完成（确保侧边栏分类已渲染）
  let retries = 0;
  while (
