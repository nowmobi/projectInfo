const Utils = {
  ...
  getArticleImage(article) {
    if (article && article.img) {
      if (/^https?:\/\//i.test(article.img)) {
        return article.img;
      }
      if (article.img.startsWith("/")) {
        return `${RemoteDataConfig.baseUrl}${article.img}`;
      }
      if (article.id) {
        return `${RemoteDataConfig.baseUrl}/${article.id}/${article.img}`;
      }
      return `${RemoteDataConfig.baseUrl}/${article.img}`;
    }
...
const RemoteDataConfig = {
  ...
  articleDataUrl(id) {
    return `${this.baseUrl}/${id}/data.json`;
  },
