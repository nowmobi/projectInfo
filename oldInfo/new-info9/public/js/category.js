...
class CategoryPage {
    ...
    renderArticles(articles, categoryName, emptyStateOptions = {}) {
        ...
        Utils.setupArticlesContainer(articlesContainer);
        ...
        const articlesHTML = Utils.generateArticleListHTML(articles, 2);
            ...
            <div class="cat-card" data-category="${category.name}">
                <div class="cat-card-icon">
                    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="M10 18V21" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M14 18V21" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
    ...
    bindArticleEvents() {
        const articleCards = document.querySelectorAll('.card-list');
        ...
        if (!query.trim()) {
            if (this.currentCategory) {
                const filteredArticles = this.articles.filter(article => {
                    ...
                    const articleId = String(article.id).trim();
                    if (articleId === "" || articleId === "undefined" || articleId === "null") return false;
                    return article.type === this.currentCategory;
                });
