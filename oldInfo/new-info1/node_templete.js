    ...
    try {
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 首先尝试匹配标准格式: Copyright © 2025 域名. All rights reserved.
                // 支持换行和空格
                ...
                // 如果上面的匹配失败，尝试匹配颜色代码格式: Copyright © 2025 #ba7ac7. All rights reserved.
                // 支持换行和空格
                if (!modified) {
                ...
                // 更新 .logo 标签内的域名（如果存在）
                const logoRegex = /(<div\s+class=["']logo["'][^>]*>)[\s\S]*?(<p>)[\s\S]*?([a-zA-Z0-9][a-zA-Z0-9.-]*\.[a-zA-Z]{2,})[\s\S]*?(<\/p>)[\s\S]*?(<\/div>)/gi;
                htmlContent = htmlContent.replace(logoRegex, (match, openTag, pOpen, oldDomain, pClose, closeTag) => {
                    if (!foundOldDomain) foundOldDomain = oldDomain;
                    modified = true;
                    return openTag + pOpen + cleanDomain + pClose + closeTag;
                });
