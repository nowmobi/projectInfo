...
class HealthNewsApp {
  ...
  async renderHomePageLayout(articlesToRender = null) {
    ...
    // 如果是搜索模式（articlesToRender存在且小于总数），即使为空也显示搜索结果容器
  ...
  renderCategoryList(containerId, articles, count) {
    ...
    cards.forEach((card, index) => {
      if (displayArticles[index]) {
        ...
        if (img) {
          img.src = this.getArticleImage(article);
          img.alt = article.title;
        }

        let content = card.querySelector(".article-content");
        if (!content) {
