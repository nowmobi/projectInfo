  ...
  async loadCategories() {
    ...
    try {
      if (
        ...
        window.financeNewsApp.categoryOrder.length > 0
      ) {
        const preloadedCategories = normaliseCategories(
      ...
      const response = await fetch(DataConfig.dbUrl, { cache: "no-store" });
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();

      if (Array.isArray(data) && data.length > 0 && data[0][this.infoKey]) {
        const categories = normaliseCategories(data[0][this.infoKey]);
