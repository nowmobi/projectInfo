...
class ArticleDetailPage {
  ...
  async loadArticleDetail(maxRetries = 3, currentRetry = 0) {
    try {
      ...
      this.renderQuickRecommendedArticles();
  ...
  async renderRecommendedArticles() {
    ...
    try {
      const recommended = await this.getRecommendedArticles(this.articleId, 6);
          ...
          return `
            <a class="article-card" href="${detailHref}">
              ...
              <div class="article-content">
                <h4 class="article-title">${Utils.decodeUnicode(articleTitle)}</h4>
                ${article.summary ? `<p class="article-summary">${article.summary.substring(0, 60)}...</p>` : ""}
  ...
  async renderQuickRecommendedArticles() {
    const quickRecommendedContainer = document.querySelector(".quick-recommended-list");
    if (!quickRecommendedContainer) return;
    quickRecommendedContainer.innerHTML = "<p>Loading...</p>";
    try {
      const recommended = await this.getRecommendedArticles(this.articleId, 3);
      if (recommended.length === 0) {
        quickRecommendedContainer.innerHTML = "<p>No related articles available.</p>";
        return;
      }
      
      quickRecommendedContainer.innerHTML = recommended
        .map((article) => {
          const articleTitle = article.title || article.headline || article.subject || "Untitled";
          const articleId = article.id || article.articleId || article.newsId;
          const detailHref = `sec.html?id=${articleId}` + (window.channel ? `&channel=${window.channel}` : "");
          return `
            <a class="quick-recommended-item" href="${detailHref}">
              <span class="dot"></span>
              <span class="quick-recommended-title">${Utils.decodeUnicode(articleTitle)}</span>
            </a>
          `;
        })
        .join("");
    } catch (error) {
      quickRecommendedContainer.innerHTML = "<p>Failed to load related articles.</p>";
    }
  }

  async renderTopArticles(count = 3) {
    ...
    topArticlesList.innerHTML = "<p>Loading...</p>";
    ...
    try {
      ...
      const validArticles = articles.filter(article => {
        ...
        return articleId && articleTitle && String(articleId) !== String(this.articleId);
      });
      ...
      topArticlesList.innerHTML = '<div style="text-align: center; color: #999; padding: 10px;">加载失败</div>';
    }
  }
