...
class HealthNewsApp {
  ...
  updateSearchUI(query) {
    ...
    if (randomArticlesSection) {
      randomArticlesSection.style.display = query.trim() ? "none" : "block";
    }

    if (topArticlesSection) {
      topArticlesSection.style.display = query.trim() ? "none" : "block";
    }
  }
  ...
  renderArticles(articlesToRender = null) {   
    ...
    if (articlesToRender) {
      articles = articlesToRender;
        ...
        articles = this.getTopArticlesByCategory(3);
      } else {
  ...
  renderQuickRecommendedArticles() {
    const quickRecommendedContainer = document.getElementById("quickRecommendedList");
    ...
    const randomArticles = this.articles
      .filter((article) => article.id != null && article.id !== "")
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    quickRecommendedContainer.innerHTML = randomArticles
      .map((article) => {
