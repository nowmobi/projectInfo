    ...
    return htmlContent.replace(
      ...
      (match, before, src, after) => {
        ...
        if (src.startsWith("http://") || src.startsWith("https://")) {
          const imageBase = remoteDataConfig.baseConfig.imageBaseUrl || remoteDataConfig.baseConfig.baseUrl;
          const targetDomain = new URL(imageBase).hostname;
          const srcUrl = new URL(src);
          
          if (srcUrl.hostname !== targetDomain) {
            srcUrl.hostname = targetDomain;
            newSrc = srcUrl.toString();
          }
        } else {
          newSrc = remoteDataConfig.buildImageUrl(articleId, src);
        }
