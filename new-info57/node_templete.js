...
function updateBaseURL(date, domain) {
    try {
        ...
        // 更新 created_at
        const createdAtRegex = /(categoryUrl:\s*["'].*?created_at=)([^&"']+)/;
...
// 更新CSS中的 --color2 变量值
function updateCSSColor2(color2) {
    try {
        const cssPath = path.join(__dirname, 'public', 'style', 'inpublic.css');
        let cssContent = fs.readFileSync(cssPath, 'utf8');
...
async function main() {
    ...
    try {
        ...
        // 2. 更新CSS变量 --color1
        updateCSSColor(config.color1 || config.color);
        console.log('');
        
        // 3. 更新CSS变量 --color2
        updateCSSColor2(config.color2 || getComplementaryColor(config.color1 || config.color));
        console.log('');
