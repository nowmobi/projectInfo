...
function readConfig() {
    try {
        ...
        // 如果没有提供 color2，则自动计算互补色
        if (!config.color2) {
            config.color2 = getComplementaryColor(config.color1);
            console.log(`自动计算互补色: ${config.color2}`);
        }
    ...
    try {
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                htmlContent = htmlContent.replace(logoRegex, (match, prefix, oldDomain, suffix) => {
                    modified = true;
