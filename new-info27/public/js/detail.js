...
class ArticleDetailPage {
  ...
  renderArticle(article) {
    ...
    if (authorElement) {
      ...
      authorElement.style.display = article.author ? '' : 'none';
    }
    
    
    const timeElement = document.getElementById('articleTime');
    if (timeElement) {
      const formattedTime = window.Utils.formatTimestamp(article.create_time) || 'Unknown Time';
    ...
    if (sourceElement) {
      ...
      sourceElement.style.display = article.source ? '' : 'none';
    }
