...
class ArticleDetailPage {
  ...
  async loadArticleDetail() {
    try {
      const detailBaseUrl = getDetailBaseUrl();
      const detailApiPath = getDetailApiPath();
      const articleUrl = `${detailBaseUrl}/${this.articleId}/${detailApiPath}/data.json`;
  ...
  renderArticleMeta(article) {
    ...
    if (categoryElement && article.author) {
    ...
    if (sourceElement && article.source) {
      const source = UnicodeDecoder.decode(article.source) || article.source || '';
  ...
  normalizeContentImages(container, articleId) {
    ...
    images.forEach((img) => {
      ...
      const fullImageUrl = `https://news-api.szwyi.com/img/${originalSrc}`;
  ...
  setupBackButton() {
    ...
    if (imageBackButton) {
      imageBackButton.addEventListener("click", (e) => {
        ...
        let homePath = imageBackButton.getAttribute('href');
        if (!homePath) {
          const isInPagesDir = window.Utils?.isInPagesDir?.() || false;
          homePath = isInPagesDir ? "../index.html" : "index.html";
        }
