...
function updateCSSColor1AndColor2(color1, color2) {
    try {
    ...
    try {
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 5. 更新 logo 中 p 标签的域名 - 改进匹配逻辑
                const logoPRegex = /(<div\s+class=["']logo(?:-[a-z]+)*["']\s*>(?:\s*<svg[^>]*>.*?<\/svg>\s*)?)\s*<p>([^<]+)<\/p>(\s*<\/div>)/gi;
                htmlContent = htmlContent.replace(logoPRegex, (match, openDiv, oldDomain, closeDiv) => {
                    const domainMatch = oldDomain.match(/[a-zA-Z0-9][a-zA-Z0-9.-]*\.[a-zA-Z]{2,}/);
                    if (domainMatch) {
                        modified = true;
                        return openDiv + '<p>' + cleanDomain + '</p>' + closeDiv;
                    }
                    return match;
                });
                
                // 6. 针对 .logo-center 的特殊处理
                const logoCenterRegex = /(<div\s+class=["']logo\s+logo-center["']>\s*)<p>([^<]+)<\/p>(\s*<\/div>)/gi;
                htmlContent = htmlContent.replace(logoCenterRegex, (match, openDiv, oldDomain, closeDiv) => {
                    const domainMatch = oldDomain.match(/[a-zA-Z0-9][a-zA-Z0-9.-]*\.[a-zA-Z]{2,}/);
                    if (domainMatch) {
                        modified = true;
                        return openDiv + '<p>' + cleanDomain + '</p>' + closeDiv;
...
async function main() {
    ...
    try {
        ...
        updateCSSColor1AndColor2(config.color1, config.color2);
        console.log('');
