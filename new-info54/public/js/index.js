class FinanceNewsApp {
  ...
  async loadData() {
    try {
      ...
      if (this.articles.length === 0 && Array.isArray(rawData)) {
        
        const hasCategoryNames = rawData.every(item => typeof item === 'string');
        if (hasCategoryNames) {
          ...
          this.categories = [
            ...
            ...rawData.map((categoryName, index) => ({
  ...
  renderCategoryBar() {
    ...
    const hotCategory = `
      <div class="cat-pill active" data-category="home">
        <div class="cat-pill-icon">
          <img src="public/images/home.png" alt="Home" />
        </div>
        <div class="cat-pill-label">Home</div>
      </div>
    `;
    ...
    const otherCategories = categories
      .map(
        ...
        <div class="cat-pill" data-category="${category.id}">
          <div class="cat-pill-icon">
            <img src="public/images/${index + 1}.png" alt="${category.name}" />
          </div>
  ...
  renderCategoriesSections() {
    ...
    categories.forEach((category, index) => {
      if (index < allSections.length) {
        ...
        this.populateCategorySection(section, category, customLimit);
      }
    });
  ...
  populateCategorySection(section, category, customLimit) {
    if (!section) return;
    ...
    if (header) {
      ...
      let icon = header.querySelector(".category-section-icon");
      if (icon) {
        icon.remove();
      }

      icon = document.createElement("img");
      icon.className = "category-section-icon";
      const categoryIndex = this.categories.findIndex(cat => cat.id === category.id);
      const iconNumber = categoryIndex > 0 ? categoryIndex : 1;
      icon.src = `public/images/${iconNumber}.png`;
    ...
    const articleLimit = customLimit !== undefined ? customLimit : (isListStyle ? 3 : 4);
    const categoryArticles = this.getArticlesByCategory(category.id, articleLimit);
