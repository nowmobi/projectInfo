class ArticleDetailPage {
      ...
      if (!remoteResponse.ok) {
        if (
          ...
          if (firstArticleId !== this.articleId) {
            ...
            const retryResponse = await fetch(retryUrl, { cache: "force-cache" });
            if (retryResponse.ok) {
      ...
      if (startCollecting) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (cloned.tagName === "IMG") {
            cloned.setAttribute("referrerpolicy", "no-referrer");
          }
      ...
      if (traverse(child)) {
        ...
        while (sibling) {
          if (sibling.nodeType === Node.ELEMENT_NODE) {
            const cloned = sibling.cloneNode(true);
  ...
  decodeUnicode(str) {
    ...
    decoded = decoded.replace(urlRegex, (url) => {
