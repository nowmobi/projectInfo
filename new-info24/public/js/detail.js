import { Category_URL, getImgUrl, getDataBaseUrl, getArticles, ARTICLE_PATH } from "./BaseURL.js";
...
class ArticleDetailPage {
  ...
  async loadArticleDetail() {
    try {
      const articleUrl = `${REMOTE_DATA_BASE_URL}/${this.articleId}/${ARTICLE_PATH}/data.json`;
    ...
    imgElements.forEach((img, index) => {
      ...
      if (
        ...
        if (article && article.id) {
          ...
          const remoteImgUrl = `${REMOTE_DATA_BASE_URL}/${article.id}/${ARTICLE_PATH}/${imgFileName}`;
  ...
  combineImagesWithParagraphs(paragraphs, images) {
    ...
    paragraphs.forEach((paragraph, index) => {
      ...
      if (index === 0 && images.length > 0) {
        chunk.elements.push(images[0].element);
      }

      chunk.elements.push(paragraph);
      chunks.push(chunk);
    });
    ...
    if (images.length > 1) {
      images.slice(1).forEach((image, index) => {
        if (chunks[index + 1]) {
          chunks[index + 1].elements.unshift(image.element);
        } else if (chunks.length > 0) {
          chunks[chunks.length - 1].elements.push(image.element);
        }
      });
    }
  ...
  normalizeContentImages(container, articleId) {
    ...
    images.forEach((img) => {
      ...
      if (normalisedId) {
        img.setAttribute(
          "src",
          `${REMOTE_DATA_BASE_URL}/${normalisedId}/${ARTICLE_PATH}/${originalSrc}`
        );
      } else {
