...
// 更新 config.json
function updateConfig(domain, color, color1, color2, info, json) {
    try {
        // 确定使用的颜色值
        ...
        let finalColor2 = (color2 && color2.trim() !== '') ? color2.trim() : finalColor1;
         ...
         // 处理 info 和 json
        // 如果 info 为空，默认为 "1"
        let finalInfo = (info && info.toString().trim() !== '') ? info.toString().trim() : "1";
        // 如果 json 为空，默认为 "1"
        let finalJson = (json && json.toString().trim() !== '') ? json.toString().trim() : "1";
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                const color2 = row.color2 || '';
                 // 获取 info 和 json 值
                const info = row.info || '1';
                const json = row.json || '1';
                ...
                // 3.1 更新 config.json
                updateConfig(domain, color, color1, color2, info, json);
