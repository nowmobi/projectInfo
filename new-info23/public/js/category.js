...
import { Category_URL, getImgUrl, getArticlesData } from './BaseURL.js';
  ...
  async loadData() {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const categoryOrder = this.extractCategoryOrder(articlesData);
        const processed = this.processData(articlesData, categoryOrder);
  ...
  async loadFallbackData() {
    ...
    if (cachedData) {
      try {
        ...
        let articlesData = parsedData;
        if (parsedData && parsedData.value && Array.isArray(parsedData.value)) {
          articlesData = parsedData.value;
        }
        const categoryOrder = this.extractCategoryOrder(articlesData);
        const processed = this.processData(articlesData, categoryOrder);
