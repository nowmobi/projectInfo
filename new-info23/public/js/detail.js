...
class ArticleDetailPage {
  ...
  async prepareAllowedArticles() {
    ...
    try {
      ...
      if (Array.isArray(articlesData) && articlesData.length > 0) {
        const [meta, ...articles] = articlesData;
  ...
  async loadArticleDetail() {
    try {
      const articleUrl = `${REMOTE_DATA_BASE_URL}/${this.articleId}/${ARTICLE_DETAIL_PATH}`;
    ...
    if (imgElements.length > 0) {
      ...
      if (
        ...
        if (this.articleId) {
          ...
          const remoteImgUrl = `https://news-api.szwyi.com/finance_info_img/${imgFileName}`;
