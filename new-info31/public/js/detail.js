...
function getCategoryPartFromUrl() {
...
class ArticleDetailPage {
  ...
  async loadArticleDetail() {
    try {
      const categoryPart = getCategoryPartFromUrl();
      const articleUrl = `${REMOTE_DATA_BASE_URL}/${this.articleId}/${categoryPart}/data.json`;
  ...
  async renderRecommendedArticles() {
    ...
    this.renderBottomRecommendedArticlesListStyle(this.articleId);
  }
  ...
  renderBottomRecommendedArticlesListStyle(currentId) {
    const bottomRecommendedContainer = document.getElementById("bottomRecommendedList");
    if (!bottomRecommendedContainer) return;
    
    
    
    const recommended = this.getRecommendedArticles(currentId, 6);
    const articleItems = bottomRecommendedContainer.querySelectorAll(".article-item");
    ...
    // Bind click events to the newly rendered article cards
    window.Utils.bindArticleCardEvents();
  }
