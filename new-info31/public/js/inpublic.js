...
const Utils = {
  ...
  createHomeArticleCard(article) {
    ...
    const author = this.escapeHtml(article.author || 'News Author');
    const source = this.escapeHtml(article.source || 'News Source');
      ...
      <div class="article-item" data-id="${articleId}">
        ...
        <div class="article-info">
          ...
          <div class="article-meta">
            ...
            ${timeStr ? `<span class="article-date">${timeStr}</span>` : ''}
          </div>
          ${source ? `<div class="article-source">${source}</div>` : ''}
        </div>
      </div>
    `;
  },
  ...
  createDetailRecommendedArticleCard(article, decodeUnicode = null) {
    ...
    return `
      <div class="article-item" data-id="${articleId}">
        ...
        <div class="article-info">
          ${categoryTag ? `<span class="category-tag">${this.escapeHtml(categoryTag)}</span>` : ''}
          <p class="article-title">${title}</p>
          <span class="article-date">${this.escapeHtml(timeStr)}</span>
        </div>
      </div>
    `;
  },
  ...
  bindArticleCardEvents(detailPagePath = null) {
    const articleCards = document.querySelectorAll(SELECTORS.articleListItem);
