...
class CategoryPage {
  ...
  renderArticles(articles, categoryName) {
    ...
    const articlesHTML = sortedArticles.map((article, index) => {
      let html = this.createArticleItemHTML(article, index + 1);
      if ((index + 1) === 3) {
        html += `
          <div class="ads">
    ...
    return `
      <div class="article-list-item" data-id="${article.id}">
        ...
        <div class="article-info">
          ...
          <p class="article-description">${window.Utils.escapeHtml(timeStr)}</p>
        </div>
      </div>
    `;
  }
    ...
    articleItems.forEach((item) => {
      item.addEventListener("click", () => {
        ...
        if (articleId && !articleId.startsWith("placeholder-")) {
          window.location.href = `../de.html?id=${articleId}`;
        }
      });
    });
  }
