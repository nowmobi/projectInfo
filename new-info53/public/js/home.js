...
import { getImgUrl, fetchCategoryData } from './BaseURL.js';
...
class Website {
  ...
  showSearchResults(count, query) {
    ...
    const aboutSectionHome = document.querySelector('.about-section-home');
    ...
    if (aboutSectionHome) {
      aboutSectionHome.style.display = 'none';
    }
  }
  ...
  hideSearchResults() {
    ...
    if (aboutSectionHome) {
      aboutSectionHome.style.display = 'block';
    }
  }
  ...
  createCarouselItemHTML(article, index) {
    ...
    const type = window.Utils.escapeHtml(article.type || 'Editor');
    const isActive = index === 0 ? 'active' : '';
    const articleId = window.Utils.escapeHtml(String(article.id || ''));
    const detailPagePath = 'de.html';
    const detailUrl = `${detailPagePath}?id=${articleId}`;
    
    return `
      <div class="carousel-item ${isActive}" data-id="${article.id}" data-index="${index}">
        <a href="${detailUrl}" class="carousel-link">
          <div class="carousel-content">
            <div class="carousel-text">
              <p class="carousel-title">${title}</p>
              <div class="carousel-meta">
                <span class="carousel-date">${window.Utils.escapeHtml(timeStr)}</span>
  ...
  appendArticlesWithAds(articlesGrid, articles) {
    ...
    this.categoryOrder.forEach(categoryName => {
      ...
      if (categoryArticles.length > 0) {
        ...
        let articlesToShow;
        if (sectionIndex === 0) {
          articlesToShow = shuffled.slice(0, 2);
        } else if (sectionIndex === 4) {
          articlesToShow = shuffled.slice(0, 8);
        } else if (sectionIndex === 5) {
          articlesToShow = shuffled.slice(0, 6);
        } else if (sectionIndex === 6) {
          articlesToShow = shuffled.slice(0, 8);
        } else if (section.classList.contains('category-section-grid')) {
          articlesToShow = shuffled.slice(0, 4);
        } else {
          articlesToShow = shuffled.slice(0, 3);
        }
