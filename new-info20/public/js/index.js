class FinanceNewsApp {
      ...
      .map((categoryName) => {
        ...
        <div class="category-section section--list">
          ...
          <div class="section-articles">
            ${Utils.generateArticleListHTML(categoryArticles, 2, true)}
          </div>
        </div>
      `;
      })
    ...
    categories.forEach((category, index) => {
      if (index < allSections.length) {
        ...
        if (!header) {
          ...
          const articlesContainer = section.querySelector(".section-articles");
          if (articlesContainer) {
        ...
        const hasFeatured = section.classList.contains("section--featured");
        const hasList = section.classList.contains("section--list");
        
        if (!hasFeatured && !hasList) {
          section.classList.remove("section--featured", "section--list");
  ...
  populateCategorySection(section, category) {
    ...
    articlesWrapper.innerHTML = categoryArticles
      .map((article, cardIndex) => {
        if (isListStyle) {
          const layout = cardIndex % 2 === 0 ? "left" : "right";
          return Utils.generateArticleListCardHTML(article, 2, layout);
        }
  ...
  bindArticleEvents() {
    const articleCards = document.querySelectorAll(".card-simple, .card-list");
  ...
  bindCategoryTitleEvents() {
    const categoryHeaders = document.querySelectorAll(".section-header");
  ...
  renderCarouselCards() {
    ...
    carouselTrack.innerHTML = articles
      .map((article, index) => {
        ...
        return `
          <div class="carousel-card" data-index="${index}" data-id="${
          article.id
        }">
            <img src="${imageUrl}" alt="${
          article.title
        }" class="carousel-bg" referrerpolicy="no-referrer" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%25%22 height=%22100%25%22%3E%3Crect fill=%22%23746097%22 width=%22100%25%22 height=%22100%25%22/%3E%3C/svg%3E'">
            <div class="carousel-content-wrapper">
              ...
              <div class="carousel-content-right">
                <img src="${imageUrl}" alt="${
          article.title
        }" class="carousel-main-img" referrerpolicy="no-referrer" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%25%22 height=%22100%25%22%3E%3Crect fill=%22%23746097%22 width=%22100%25%22 height=%22100%25%22/%3E%3C/svg%3E'">
              </div>
            </div>
          </div>
        `;
  ...
  initializeHorizontalScroll() {
    ...
    const validArticles = this.articles.filter((article) => article.id);
    if (validArticles.length === 0) {
      document.getElementById("horizontalScrollContainer").style.display =
        "none";
      return;
    }

    const shuffled = [...validArticles].sort(() => Math.random() - 0.5);
    const scrollArticles = shuffled.slice(0, 12);

    track.innerHTML = scrollArticles
      .map((article) => {
        const imageUrl = Utils.getArticleImage(article);
        return `
          <div class="horizontal-scroll-item" data-id="${article.id}">
            <img src="${imageUrl}" alt="${
          article.title
        }" referrerpolicy="no-referrer" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%25%22 height=%22100%25%22%3E%3Crect fill=%22%23746097%22 width=%22100%25%22 height=%22100%25%22/%3E%3C/svg%3E'">
            <div class="horizontal-scroll-item-title">${
              article.title || ""
            }</div>
          </div>
        `;
...
function initSearchFunctionality() {
  ...
  const showSearchBar = () => {
    ...
    searchInput.focus();
    if (stackedCarousel) {
      stackedCarousel.style.display = "none";
    }
  ...
  const hideSearchBar = () => {
    ...
    if (stackedCarousel) {
      stackedCarousel.style.display = "block";
    }
    if (horizontalScrollContainer) {
