...
export function initAdsense() {
    ...
    const rawHomeSlots = firstAdUnit.home || [];

    // Adsense SDK
    loadAdSenseSDK(clientId, function () {
        // ========== home ==========
        let homeSlots = rawHomeSlots.map(item => item.slot);
        if (adConf.randad !== 0) {
            homeSlots = shuffleArray(homeSlots);
            console.log("home ad is rand", homeSlots);
        }
        homeSlots.forEach((slot, index) => {
            const targetId = `div-gpt-ad-home${index + 1}`;
            const wrapDom = document.getElementById(targetId);
            if (!wrapDom) {
                console.warn(`can not found home div #${targetId}`);
                return;
            }
...
// homegg_ads.js 由 index.html 内联模块脚本动态创建，
