...
// 更新 BaseURL.js 中的 baseUrl 和 categoryUrl 的 API 域名
...
function updateBaseURL(domain) {
    try {
        ...
        if (!fs.existsSync(baseUrlPath)) {
            console.log(`○ BaseURL.js 不存在，跳过: ${path.relative(__dirname, baseUrlPath)}`);
            return;
        }
...
async function main() {
    ...
    try {
        ...
        // 5. 更新 BaseURL.js 接口域名（news-api.szwyi.com → api.{一级域名}）
