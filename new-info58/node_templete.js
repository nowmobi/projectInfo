...
function updateBaseURL(date, domain) {
    try {
        ...
        // 更新 created_at
        const createdAtRegex = /(Category_URL\s*=\s*["'].*?created_at=)([^&"']+)/;
    ...
    try {
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 匹配 "© 2025 域名 All rights reserved." 格式（无点号）
                const copyrightRegex4 = /(©\s+\d{4}\s+)([a-zA-Z0-9][a-zA-Z0-9.-]*\.[a-zA-Z]{2,})(\s+All\s+rights?\s+reserved\.)/gi;
                htmlContent = htmlContent.replace(copyrightRegex4, (match, prefix, oldDomain, suffix) => {
                    modified = true;
                    return prefix + cleanDomain + suffix;
                });

                // 6. 更新 <link rel="preconnect" href="https://api.xxx" /> 中的 api 域名
                // 与 BaseURL.js 中 api.<second_level_domain> 的规则保持一致
                // 仅匹配 api. 开头的域名，不会误伤 Google Fonts (fonts.googleapis.com / fonts.gstatic.com)
                if (domain) {
                    const newDomainPart = extractDomainParts(domain);
                    const preconnectApiRegex = /(<link\s+rel=["']preconnect["']\s+href=["']https?:\/\/api\.)([a-zA-Z0-9][a-zA-Z0-9.-]*\.[a-zA-Z]{2,})(["'])/gi;
                    htmlContent = htmlContent.replace(preconnectApiRegex, (match, prefix, oldDomain, suffix) => {
                        modified = true;
                        return prefix + newDomainPart + suffix;
                    });
                }
