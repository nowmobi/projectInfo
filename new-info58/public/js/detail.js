...
class ArticleDetailPage {
  ...
  async init() {
    ...
    // If no id in URL, randomly select one
    if (
      !this.articleId ||
    ...
    // If id is provided in URL, use it directly (don't check if it's in the list)
    // The actual article data might exist even if it's not in the current article list
  ...
  async loadArticleDetail() {
    try {
      const articleUrl = Detail_URL_TEMPLATE.replace('{id}', this.articleId);
  ...
  normalizeContentImages(container, articleId) {
    ...
    images.forEach((img) => {
      ...
      const fullImageUrl = `${IMG_BASE_URL}/${originalSrc}`;
  ...
  setupBackButton() {
    // #imageBackButton 已是 <a href="index.html">，浏览器原生跳转即可
    // 保留函数以兼容既有调用链（detail.js 初始化时调用）
    const imageBackButton = document.getElementById("imageBackButton");
    if (!imageBackButton) return;
    // 仅作为 fallback：若将来出现非 <a> 的返回按钮，再走 JS 跳转
    if (imageBackButton.tagName.toLowerCase() === 'a') return;
