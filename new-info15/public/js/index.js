  ...
  renderCategoryBar() {
    ...
    this.categories.forEach((category, index) => {
      ...
      if (index === 0 && category.id === 'all') {
        iconChar = HOME_CATEGORY_ICON;
      } else {
        const iconIndex = (index - 1) % CATEGORY_ICON_POOL.length;
        const safeIndex = iconIndex >= 0 ? iconIndex : (iconIndex + CATEGORY_ICON_POOL.length);
        iconChar = CATEGORY_ICON_POOL[safeIndex] || DEFAULT_CATEGORY_ICON;
      }
      ...
      button.className = `category-btn ${isActive}`;
