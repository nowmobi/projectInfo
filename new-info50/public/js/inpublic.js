...
class HealthNewsApp {
  ...
  showSearchResults(count, query) {
    ...
    if (randomArticlesSection && topArticlesSection) {
      randomArticlesSection.style.display = "none";
  ...
  hideSearchResults() {
    ...
    if (randomArticlesSection && topArticlesSection) {
      randomArticlesSection.style.display = "";
      topArticlesSection.style.display = "";
    }
  }
    ...
    topArticlesList.innerHTML = topArticles.map((article, index) => {
      ...
      return `
        <div class="top-article-item" onclick="window.location.href='${detailHref}'">
          <span class="top-article-number">${index + 1}</span>
