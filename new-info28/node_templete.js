...
function updateDomainInHTML(domain) {
    try {
        ...
        const htmlFiles = [
            ...
            path.join(__dirname, 'depth.html'),
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 3. 更新logo中的p标签内容，确保与footer-copyright中的域名一致
                const logoPTagRegex = /(<div\s+class=["']logo["']>\s*<p>)([^<]+)(<\/p>\s*<\/div>)/gi;
                htmlContent = htmlContent.replace(logoPTagRegex, (match, openTag, oldContent, closeTag) => {
                    modified = true;
