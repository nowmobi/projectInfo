import {
  ...
  getRemoteDbUrl,
  getArticleDetailUrl,
  getFinanceInfoImgUrl,
} from "./BaseURL.js";

const REMOTE_DATA_BASE_URL = getDataBaseUrl();
const REMOTE_DB_URL = getRemoteDbUrl();
...
class ArticleDetailPage {
  ...
  renderArticleMeta(article) {
    ...
    if (timeElement && article.create_time) {
      const formattedTime = window.Utils?.formatTimestamp?.(article.create_time) || article.create_time;
  ...
  extractImages(htmlElement) {
    ...
    if (imgElements.length > 0) {
      ...
      if (
        ...
        const remoteImgUrl = getFinanceInfoImgUrl(imgFileName);
  ...
  normalizeContentImages(container, articleId) {
    ...
    images.forEach((img) => {
      ...
      const fullImageUrl = getFinanceInfoImgUrl(originalSrc);
