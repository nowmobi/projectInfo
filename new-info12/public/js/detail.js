...
const SELECTORS = {
  ...
  articleSourceValue: "#articleSourceValue",
...
class UnicodeDecoder {
  static decode(str) {
    ...
    decoded = this.protectUrls(decoded);
    ...
    decoded = this.restoreUrls(decoded);
  ...
  static protectUrls(str) {
    const urlPlaceholders = [];
    const urlPattern = /(src|href|data-src)=["']([^"']+)["']/gi;
    
    str = str.replace(urlPattern, (match, attr, url) => {
      const placeholder = `__URL_PLACEHOLDER_${urlPlaceholders.length}__`;
      urlPlaceholders.push({ attr, url });
  ...
  static restoreUrls(str) {
    if (!this.urlPlaceholders || this.urlPlaceholders.length === 0) {
      return str;
    }

    this.urlPlaceholders.forEach(({ attr, url }, index) => {
      const placeholder = `__URL_PLACEHOLDER_${index}__`;
      str = str.replace(new RegExp(`"${placeholder}"`, 'g'), `"${url}"`);
    });
...
class ArticleDetailPage {
  ...
  async prepareAllowedArticles() {
    ...
    if (this.isLoadingArticles) {
      ...
      let waited = 0;
      while (this.isLoadingArticles && waited < maxWait) {
    ...
    try {
      const response = await fetchWithTimeout(REMOTE_DB_URL, {}, 10000);
      if (response.ok) {
  ...
  async loadArticleDetail() {
    try {
      const articleUrl = getDetailApiUrl(this.articleId);
  ...
  renderArticleSource(article) {
    ...
    const sourceValueElement = document.querySelector(SELECTORS.articleSourceValue);
    
    if (!sourceElement || !sourceValueElement) return;
    ...
    if (article.source) {
      ...
      this.updateElementText(SELECTORS.articleSourceValue, source);
  ...
  extractImages(htmlElement) {
    ...
    if (imgElements.length > 0) {
      ...
      if (imgSrc && !this.isAbsoluteOrRelativeUrl(imgSrc)) {
        if (this.articleId && REMOTE_DATA_BASE_URL) {
  ...
  normalizeContentImages(container, articleId) {
    ...
    const normalizedId = articleId ? String(articleId) : this.articleId;
