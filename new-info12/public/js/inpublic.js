...
const Utils = {
  ...
  createDetailRecommendedArticleCard(article, decodeUnicode = null) {
    ...
    return `
      <div class="article-card article-detail-recommended" data-id="${articleId}">
        <div class="article-image">
          <img src="${this.escapeHtml(imageUrl)}" alt="${title}" onerror="this.style.display='none';">
        </div>
        <div class="article-content">
...
class SidebarManager {
  constructor() {
    ...
    this.isDetailPage = this.checkIsDetailPage();
    this.init();
  }
  ...
  checkIsDetailPage() {
    const pathname = window.location.pathname;
    return pathname.includes('detail.html') || pathname.includes('/detail.html');
  }
  ...
  async loadCategories() {
    ...
    try {
      ...
      if (window.website && window.website.isLoading !== undefined) {
        isLoading = window.website.isLoading;
      }
      
      while (!categoryOrder && waitedTime < maxWaitTime) {
        
        if (window.website) {
          
          if (window.website.isLoading === true) {
          ...
          if (window.website.categoryOrder && Array.isArray(window.website.categoryOrder) && window.website.categoryOrder.length > 0) {
            categoryOrder = window.website.categoryOrder;
            break;
          }
        }
        
        
        if (this.isDetailPage && window.articleDetailPage && window.articleDetailPage.allArticles && Array.isArray(window.articleDetailPage.allArticles) && window.articleDetailPage.allArticles.length > 0) {
          
          const typeSet = new Set();
          window.articleDetailPage.allArticles.forEach(article => {
            if (article.type) {
              typeSet.add(article.type);
            }
          });
          if (typeSet.size > 0) {
            categoryOrder = Array.from(typeSet);
            break;
          }
        }
      ...
      if (!categoryOrder) {
        ...
        if (window.website && window.website.categoryOrder && Array.isArray(window.website.categoryOrder) && window.website.categoryOrder.length > 0) {
          categoryOrder = window.website.categoryOrder;
        } else {
