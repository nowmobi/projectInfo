import { BASE_URL, getImgUrl, IMG_BASE_URL, Category_URL, Detail_URL, Detail_Article_Path } from "./BaseURL.js";
...
class ArticleDetailPage {
  ...
  async loadArticleDetail() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (articles && articles.length > 0) {
            ...
            this.articleId = articles[randomIndex].id;
          }
        }
      } catch (error) {
    ...
    if (!this.articleId) {
      this.showError("Failed to Load Article: No article ID specified");
      return;
    }

    try {
      const healthsUrl = `${Detail_URL}/${this.articleId}${Detail_Article_Path}`;
      ...
      if (healthsResponse.ok) {
        ...
        this.showError("Failed to Load Article: Article not found");
      }
    } catch (error) {
  ...
  distributeContentToArticleItems(contentContainer, htmlContent, article) {
    ...
    if (firstImage && !firstImage.src.startsWith('http')) {
      ...
      firstImage.src = `${IMG_BASE_URL}${firstImage.src}`;
    }
  ...
  async renderRecommendedArticles() {
    try {
      ...
      console.error("Failed to render recommended articles:", error);
    }
  }
      ...
      if (response.ok) {
        ...
        if (articles && articles.length > 0) {
          ...
          const recommended = articles
            ...
            .slice(0, 4);
          this.renderRecommendedArticlesList(recommended);
        }
      }
    } catch (error) {
  ...
  renderRecommendedArticlesList(articles) {
    const recommendedContainer = document.querySelector("#recommendedArticles");
    if (!recommendedContainer) return;

    recommendedContainer.innerHTML = articles
      .map(
        (article) => `
    ...
    recommendedCards.forEach((card) => {
      card.addEventListener("click", (e) => {
        ...
        if (articleId) {
          e.preventDefault();
          e.stopPropagation();
          const detailUrl = `detail.html?id=${articleId}` + (window.channel ? "&channel=" + window.channel : "");
