import { getCategoryOrder, Category_URL, getImgUrl } from "./BaseURL.js";
...
class HealthNewsApp {
  ...
  async init() {
    ...
    this.renderArticles();
    this.renderRandomFeaturedArticle();
  }
  ...
  async generateCategoriesFromArticles() {
   ...
   const categoryOrder = await getCategoryOrder(this.articles);
    ...
    articles.forEach((article) => {
      ...
      <div class="article-card-new" data-id="${
        ...
        <div class="article-time-new">${this.formatTime(
          ...
          <div class="article-excerpt-new">${this.getArticleExcerpt(article)}</div>
  ...
  renderCategoriesSections() {
    ...
    categoryList.forEach((category, index) => {
      ...
      const limit = homeArticleItems[index] 
        ? parseInt(homeArticleItems[index].dataset.limit) || 3 
        : 3;
      ...
      const sectionHtml = `
        <div class="category-section" data-category="${categoryId}">
          <div class="category-section-header">
            <h2>${truncateCategoryName(categoryName)}</h2>
          </div>
      ...
      if (index < homeArticleItems.length) {
        ...
        if ([0, 2, 4, 6].includes(index)) {
          if (categoryArticles.length > 0) {
              ...
              <div class="category-section" data-category="${categoryId}">
                ...
                <div class="category-section-articles" id="${categoryId}-articles">
                  ${categoryArticles.map((article) => `
                    <div class="article-card-image-only" data-id="${article.id}">
                      <div class="article-category-tag">${this.decodeUnicode(article.type) || "Health"}</div>
                      <h3 class="article-title-image-only">${article.title}</h3>
            ...
            `;
            homeArticleItems[index].innerHTML = imageOnlyHtml;
          } else {
  ...
  bindArticleEvents() {
    ...
    if (categoriesContainer) {
      categoriesContainer.addEventListener("click", (e) => {
        const articleCard = e.target.closest(".article-card-new, .article-card-image-only, .featured-article-card");
    ...
    if (articlesContainer) {
      articlesContainer.addEventListener("click", (e) => {
        const articleCard = e.target.closest(
          ".article-card-new, .article-card-simple, .article-card-image-only, .featured-article-card"
        );
  ...
  renderRandomFeaturedArticle() {
    const container = document.getElementById('random-featured-article');
    if (!container) return;

    const featuredContent = container.querySelector('.featured-content');
    ...
    if (validArticles.length === 0) {
      featuredContent.innerHTML = `
        <div class="empty-featured">
          ...
          <div class="empty-text">No valid articles available</div>
        </div>
      `;
      return;
    }
    ...
    const randomArticles = shuffled.slice(0, 7);

    const articleHTML = randomArticles.map((article) => `
      <div class="featured-article-card" data-id="${article.id}">
        <div class="featured-article-image">
          <img src="${getImgUrl(article)}" alt="${article.title}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
          <div class="placeholder-image" style="display: none; width: 100%; height: 100%; background: linear-gradient(135deg, #746097 0%, #7bb3d4 100%); color: white; align-items: center; justify-content: center; font-size: 12px; text-align: center; padding: 8px;">
            <div>
        ...
        <div class="featured-article-title">
          <h3>${this.decodeUnicode(article.title)}</h3>
        </div>
      </div>
    `).join('');

    featuredContent.innerHTML = articleHTML;
  }
