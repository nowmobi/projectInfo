...
class ArticleDetailPage {
  ...
  renderArticle(article) {
    ...
    this.setElementText("articleMeta", `Published on ${article.create_time ? formatTime(article.create_time) : '-'} · by ${article.author || '-'}`);
  ...
  getRecommendedArticles(currentId, count) {
    // 过滤掉当前文章，并只保留有效文章（有ID和标题）
    const validArticles = this.articles.filter(
      (article) =>
        article &&
      ...
      <div class="grid-card" data-id="${article.id}">
        ...
        <div class="article-content">
          ...
          <div class="article-meta">
            <span class="article-time">${article.create_time ? formatTime(article.create_time) : ''}</span>
          </div>
        </div>
      </div>
    `;
  ...
  renderDetailPageRecommendedArticles(currentId) {
    ...
    recommendedContainer.classList.add("category-grid");
    
    const recommended = this.getRecommendedArticles(currentId, 12);
...
document.addEventListener("DOMContentLoaded", async () => {
  initCommonSidebar();
  await renderDropdownCategories(getPageBasePath());
