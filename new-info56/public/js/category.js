...
class CategoryPage {
  ...
  renderArticles(articles, categoryName) {
    ...
    articlesContainer.classList.add("category-grid");
    ...
    const articlesHTML = sortedArticles
      .map(
        (article, index) => {
          ...
          if (index === 3) {
            html += `
                    <div class="ads" style="grid-column: 1 / -1;">
                        <div id="div-gpt-ad-category2">2</div>
                    </div>
                `;
          }
          return html;
    ...
    articlesContainer.innerHTML = `<h2 class="category-title" id="categoryTitle">${categoryName}</h2>` + articlesHTML;
  ...
  bindArticleEvents() {
    const articleCards = document.querySelectorAll(".article-card, .grid-card");
      ...
      if (href && href.includes("category.html?type=")) {
        ...
        if (type && window.categoryPage) {
          ...
          if (currentPath.includes("/pages/")) {
            ...
            window.location.href = `pages/category.html?type=${encodeURIComponent(type)}`;
          }
          return;
        }
      }
