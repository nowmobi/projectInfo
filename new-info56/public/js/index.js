...
class HealthNewsApp {
    ...
    try {
      ...
      const dropdownCategoriesHtml = categoryOrder
        .map((categoryConfig) => {
          ...
          return `
            <a href="${basePath}category.html?type=${encodeURIComponent(
              categoryName
  ...
  renderSearchResults(articles) {
    ...
    searchContainer.innerHTML = `
      <div class="category-news-list">
        ${validArticles
          .map((article, index) => this.createListCardTemplate(article, index, false, false, 2))
          .join("")}
      </div>
    `;
  ...
  createListCardTemplate(article, index = 0, isSecondSection = false, isFourthSection = false, sectionIndex = 0) {
    // 第2个section特殊样式：每个文章占一行，只显示标题和更新时间，左边有01,02,03序号
    if (isSecondSection) {
      const rankNum = String(index + 1).padStart(2, '0');
      return `
        <div class="article-card second-section-card" data-id="${article.id}">
          <span class="article-rank">${rankNum}</span>
          <div class="article-content">
            <h3 class="article-title">${article.title}</h3>
            <span class="article-time">${this.formatTime(article.create_time)}</span>
          </div>
        </div>
      `;
    }
    
    // 第3个section特殊样式：一行一个文章，上面图片，下面来源，来源下面是标题，标题下面是更新时间
    if (sectionIndex === 2) {
      return `
        <div class="article-card third-section-card" data-id="${article.id}">
          <div class="article-image">
          ...
          <div class="article-content">
            ...
            <h3 class="article-title">${article.title}</h3>
            <span class="article-time" style="display: flex; justify-content: space-between; width: 100%;">${this.formatTime(article.create_time)} <span class="arrow-right">→</span></span>
          </div>
        </div>
      `;
    }
        ...
        <div class="article-content">
          ...
          <div class="article-info">
            <div class="article-type-wrapper">
              <span class="article-type">${article.type}</span>
              ${!isSixthOrEighthSection ? `<span class="article-time">${this.formatTime(article.create_time)}${sectionIndex === 2 ? ' <span class="arrow-right">›</span>' : ''}</span>` : ''}
            </div>
            <h3 class="article-title">${article.title}</h3>
          </div>
  ...
  createGridCardTemplate(article, index = 0, sectionIndex = 0) {
    const isSpecialSection = sectionIndex >= 3 && sectionIndex <= 6;
    ...
    const rankClass = index < 3 ? 'top3' : '';
    const rankNum = sectionIndex === 1 ? String(index + 1).padStart(2, '0') : (index + 1);
    const isSixthOrEighthSection = sectionIndex === 5 || sectionIndex === 6;
    const imageStyle = isSixthOrEighthSection ? 'width: 96px; height: 96px; object-fit: cover;' : '';
    return `
      <div class="grid-card" data-id="${article.id}" ${isSixthOrEighthSection ? 'style="height: 96px; display: flex; align-items: center;"' : ''}>
        <div class="article-image" ${isSixthOrEighthSection ? 'style="width: 96px; height: 96px; flex-shrink: 0;"' : ''}>
          <img src="${this.getArticleImage(article)}" alt="${article.title}" style="${imageStyle}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'; this.nextElementSibling.style.width='96px'; this.nextElementSibling.style.height='96px';">
          <div class="placeholder-image" style="display: none; width: 100%; height: 100%; background: linear-gradient(135deg, #ba7ac7 0%, #9b6aa8 100%); color: white; align-items: center; justify-content: center; font-size: 14px; text-align: center; padding: 10px;">
            <div>
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="margin-bottom: 8px;">
                <path d="M21 19V5C21 3.9 20.1 3 19 3H5C3.9 3 3 3.9 3 5V19C21 20.1 3.9 21 5 21H19C20.1 21 21 20.1 21 19ZM8.5 13.5L11 16.51L14.5 12L19 18H5L8.5 13.5Z" fill="currentColor"/>
  ...
  /**
   * 渲染第一个section（特殊样式：左边图片，右边标题+时间）
   * @param {string} containerId - 容器ID
   * @param {Array} articles - 文章数组
   * @param {number} count - 显示的文章数量
   */
  renderFirstSection(containerId, articles, count) {
    ...
    const cardsHTML = articlesToRender
      .map((article) => this.createFirstSectionCard(article))
      .join("");
  ...
  /**
   * 生成第一个section卡片的HTML模板
   * @param {Object} article - 文章对象
   * @returns {string} HTML模板字符串
   */
  createFirstSectionCard(article) {
    return `
      <div class="first-section-card" data-id="${article.id}">
        ...
        <div class="first-section-content">
          <h3 class="first-section-title">${article.title}</h3>
          <span class="first-section-time">${this.formatTime(article.create_time)}</span>
        </div>
      </div>
    `;
  }
  ...
  renderCategoryList(containerId, articles, count, isSecondSection = false, isFourthSection = false, sectionIndex = 0) {
    ...
    if (isSecondSection) {
      const cardsHTML = articlesToRender
        .map((article, index) => this.createListCardTemplate(article, index, isSecondSection, isFourthSection, sectionIndex))
        .join("");

      const category = containerId.replace('List', '').replace(/([A-Z])/g, ' $1').trim().toUpperCase();
      const headerHTML = `<div class="second-section-header">${category}</div>`;
      container.innerHTML = `<div class="second-section-wrapper">${headerHTML}${cardsHTML}</div>`;
    } else {
      const cardsHTML = articlesToRender
        .map((article, index) => this.createListCardTemplate(article, index, isSecondSection, isFourthSection, sectionIndex))
        .join("");
  ...
  renderCategoryGrid(containerId, articles, count, isSecondSection = false, isFourthSection = false, sectionIndex = 0) {
    ...
    if (isSecondSection) {
      ...
      const cardsHTML = articlesToRender.map((article, index) => {
        ...
        if (index === 0) {
          html += '<div class="ads" id="div-gpt-ad-home4">4</div>';
        } else if (index === 1) {
          html += '<div class="ads" id="div-gpt-ad-home5">5</div>';
        }
  ...
  renderFeaturedArticle(articles) {
    ...
    featuredContainer.innerHTML = `
      <div class="featured-article-card" data-id="${randomArticle.id}">
        <div class="featured-article-image-wrapper">
          <img src="${this.getArticleImage(randomArticle)}" alt="${randomArticle.title}" class="featured-article-image" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
          <div class="placeholder-image" style="display: none; width: 100%; height: 100%; background: linear-gradient(135deg, #ba7ac7 0%, #9b6aa8 100%); color: white; align-items: center; justify-content: center; font-size: 14px; text-align: center; padding: 10px;">
            <div>
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="margin-bottom: 8px;">
                <path d="M21 19V5C21 3.9 20.1 3 19 3H5C3.9 3 3 3.9 3 5V19C21 20.1 3.9 21 5 21H19C20.1 21 21 20.1 21 19ZM8.5 13.5L11 16.51L14.5 12L19 18H5L8.5 13.5Z" fill="currentColor"/>
          ...
          <span class="featured-article-category">${randomArticle.type || 'Uncategorized'}</span>
        </div>
        <div class="featured-article-content">
          <h3 class="featured-article-title">${randomArticle.title}</h3>
          <p class="featured-article-source">${randomArticle.source || 'Unknown Source'}</p>
        </div>
      </div>
    `;
  ...
  async renderAllCategories(articles) {
    this.renderFeaturedArticle(articles);
    ...
    categoryOrder.forEach((categoryConfig) => {
      ...
      const isSpecialSection = sectionIndex >= 3 && sectionIndex <= 6;
      const useGridLayout = isSpecialSection || (layout === "grid" && !isSecondSection && !isThirdSection);
      const containerId =
      ...
      const articleCount = sectionIndex === 0 ? 4 : (sectionIndex === 1 ? 4 : (sectionIndex === 2 ? 2 : (isSpecialSection ? 4 : (useGridLayout ? 4 : 3))));
      const isFourthSection = sectionIndex === 4;
      
      if (sectionIndex === 0) {
        this.renderFirstSection(containerId, shuffledArticles, articleCount);
  ...
  bindArticleEvents() {
    const articleCards = document.querySelectorAll(
      ".article-card, .carousel-item, .ai-card, .grid-card, .search-result-item, .featured-article-card, .first-section-card"
    );
