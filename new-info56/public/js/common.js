import { getCategoryOrder } from "./BaseURL.js";

export function getPageBasePath() {
  return window.location.pathname.includes("/pages/") ? "../" : "";
}

export async function renderDropdownCategories(basePath = getPageBasePath()) {
  ...
  try {
    ...
    dropdownCategories.innerHTML = categoryOrder
      .map((categoryConfig) => {
        const categoryName =
        ...
        return `
          <a href="${basePath}pages/category.html?type=${encodeURIComponent(
            categoryName
...
export function bindDropdownNavigation() {
  ...
  if (!dropdownItemsContainer || dropdownItemsContainer.dataset.navBound) {
    return;
  }

  dropdownItemsContainer.dataset.navBound = "true";
  dropdownItemsContainer.addEventListener("click", (e) => {
    const item = e.target.closest(".dropdown-item");
    if (!item) return;

    if (item.dataset.category && document.body.classList.contains("home-page")) {
      return;
    }
...
async function autoDetectAndInitSimplePages() {
  ...
  const isTermsPage = currentPath.includes("terms.html");
  
  if (isAboutPage || isPrivacyPage || isContactPage || isTermsPage) {
