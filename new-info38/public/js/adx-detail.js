...
class ArticleDetailPage {
  ...
  async loadArticleDetail() {
    try {
      const articleUrl = getFinanceInfoUrl(this.articleId);
    ...
    if (imgElements.length > 0) {
      ...
      if (
        ...
        if (this.articleId) {
          ...
          const remoteImgUrl = getFinanceInfoImageUrl(this.articleId, imgFileName);
  ...
  normalizeContentImages(container, articleId) {
    ...
    images.forEach((img) => {
      ...
      const fullImageUrl = normalizedId
        ? getFinanceInfoImageUrl(normalizedId, originalSrc)
        : `${getDataBaseUrl()}/${originalSrc}`;
  ...
  renderHotTopics() {
    const hotTopicSection = document.getElementById('hotTopicSection');
    if (!hotTopicSection) return;
    
    if (!Array.isArray(this.allArticles) || this.allArticles.length === 0) {
      hotTopicSection.innerHTML = '<div class="loading-state">Loading hot topics...</div>';
      return;
    }

    
    const validArticles = this.allArticles.filter(article => {
      return article && 
    ...
    if (validArticles.length === 0) {
      hotTopicSection.innerHTML = '<div class="loading-state">No hot topics available...</div>';
      return;
    }

    
    const shuffled = [...validArticles].sort(() => Math.random() - 0.5);
    const hotTopics = shuffled.slice(0, 3); // 随机推荐3篇文章
    
    hotTopicSection.innerHTML = `
      <div class="recommended-articles-list">
        ${hotTopics.map(article => {
    ...
    this.bindHotTopicEvents();
  }

  
  bindHotTopicEvents() {
    const hotTopicItems = document.querySelectorAll('.recommended-article-item');
    hotTopicItems.forEach(item => {
