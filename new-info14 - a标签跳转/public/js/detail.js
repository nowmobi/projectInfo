...
class UnicodeDecoder {
  ...
  static replaceQuestionMarkEntities(str) {
    ...
    urlPlaceholders.forEach((url, index) => {
      str = str.replace(`__URL_PLACEHOLDER_${index}__`, url);
    });
...
class ArticleDetailPage {
  ...
  renderArticle(article) {
    ...
    // 显示作者
    if (article.author) {
      this.updateElementText("#articleAuthor", article.author);
    }
    ...
    if (article.create_time) {
      ...
      if (timeStr) {
        this.updateElementText("#articleTime", timeStr);
      }
    }
    
    if (article.source) {
      this.updateElementText("#articleSource", article.source);
    }
  ...
  renderMainImageFromContent() {
    setTimeout(() => {
      ...
      if (firstImg) {
        ...
        if (imageSrc) {
          console.log("Found image in content, setting main image:", imageSrc);
