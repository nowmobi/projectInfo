...
class Website {
  ...
  renderFeaturedBanner() {
    ...
    if (validArticles.length === 0) {
      carouselWrapper.innerHTML = this.createLoadingStateHTML('No valid articles available...');
      carouselIndicators.innerHTML = '';
      return;
    }
    ...
    const featuredArticles = shuffled.slice(0, Math.min(CONFIG.maxFeaturedArticles, validArticles.length));
    ...
    this.bindCarouselItemEvents();
  }

  
  bindCarouselItemEvents() {
  ...
  createCarouselItemHTML(article, index) {
    ...
    return `
      <div class="carousel-item ${isActive}" data-id="${articleId}" data-index="${index}">
